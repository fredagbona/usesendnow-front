...
```http
X-Team-Id: <uuid équipe>
```

- **Avec** `X-Team-Id` : l'espace **partagé** de cette équipe (vous devez être membre actif).
- **Sans** : espace **personnel**.

Les routes **`/api/auth/*`** ignorent cet en-tête. La gestion équipe sous **`/api/teams`** est à part ; voir [Équipes & espaces de travail](/fr/guides/teams-workspaces) et [Contexte équipe](/fr/api-reference/teams/team-context).
