...
Voir [Authentification](/fr/authentication) pour le format exact et les règles de sécurité.

### Espace équipe (`msgf_team_…`)

Les clés d'équipe exigent **`X-Team-Id`** cohérent avec l'équipe de la clé ; les routes **`/api/v1/*`** sur les données utilisent alors l'espace partagé et le plan du propriétaire d'équipe. Les clés personnelles ne doivent pas envoyer `X-Team-Id` (`TEAM_KEY_REQUIRED`). Le compte, l'usage compact et la plupart des lectures facturation exigent une clé personnelle (`TEAM_KEY_NOT_ALLOWED` avec une clé d'équipe). Détails : [Contexte équipe](/fr/api-reference/teams/team-context).
...
Voir [Codes d'erreur](/fr/resources/error-codes) pour les principaux codes.
...
| `GET` | `/api/v1/contacts/bulk-jobs` | Lister les jobs bulk contacts |
| `GET` | `/api/v1/contacts/bulk-jobs/{jobId}` | Détail d'un job bulk |
| `GET` | `/api/v1/contacts/bulk-jobs/{jobId}/progress` | Progression d'un job bulk |
| `POST` | `/api/v1/contacts/bulk-jobs/{jobId}/cancel` | Annuler un job bulk |
