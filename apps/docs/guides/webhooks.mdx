...
```bash
curl https://api.msgflash.com/api/v1/webhooks \
...
```bash
curl -X POST https://api.msgflash.com/api/v1/webhooks \
  -H "x-api-key: msgf_live_your_api_key_here" \
...
```bash
curl -X DELETE https://api.msgflash.com/api/v1/webhooks/WEBHOOK_ID \
  -H "x-api-key: msgf_live_your_api_key_here"
```
